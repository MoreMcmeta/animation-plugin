...
public final class AnimationComponentBuilder implements ComponentBuilder {
    ...
    build(AnalyzedMetadata metadata, FrameGroup<? extends MutableFrameView> baseFrames) {
        ...
        if (!(metadata instanceof AnimationGroupMetadata)) {
        ...
        AnimationGroupMetadata animationGroupMetadata = (AnimationGroupMetadata) metadata;
        ...
        return new AnimationGroupComponent(
                components,
                sortedAnimations.stream().<Runnable>map((animation) -> animation::close).collect(Collectors.toList())
        );
    }
